...
try:
    ...
    if xgboost.__version__ != xgboost_ver:
        raise Exception(f"To preserve rank ordering we require xgboost=={xgboost_ver}, but have xgboost=={xgboost.__version__}")
...
def trial_runner(trial):
    ...
    if trial.task.problem in ["binary", "multiclass"]:
        if trial.method.name == "xgboost-base":
            ...
            est = Pipeline([("ct", ct), ("est", APLRClassifier(v=0.5, num_first_steps_with_linear_effects_only=400, boosting_steps_before_interactions_are_allowed=500))])
        ...
        if trial.method.name == "xgboost-base":
            ...
            est = Pipeline([("ct", ct), ("est", APLRRegressor(v=0.5, num_first_steps_with_linear_effects_only=400, boosting_steps_before_interactions_are_allowed=500))])
        else:
            raise RuntimeError(f"Method unavailable for {trial.method.name}")
    ...
    if (trial.method.name=="aplr-base"):
        ...
        try:
            ...
            benchmark_results=pd.concat([benchmark_results,benchmark.results()], ignore_index=True).drop_duplicates(subset=["method","task","name"])
        except:
...
fit_times.to_excel("fit_times.xlsx")