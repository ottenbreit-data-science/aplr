...
#### calculate_custom_hessian_function (default = None)
A Python function that calculates the hessian if ***loss_function*** is "custom_function". The hessian should be the second order differentiation of the custom loss function (***calculate_custom_loss_function***) with respect to the predictions. Example:

```
def custom_hessian_function(y, predictions, group, other_data):
    hessian_for_mse = np.ones(len(y))
    return hessian_for_mse
```
